...
export default function SearchModal(): React.ReactElement | null {
  ...
  useEffect(() => {
    if (searchOpen && inputRef.current) { inputRef.current.focus(); setQuery(''); }
  }, [searchOpen]);
  ...
  const handleSelect = (item: SearchItem): void => { setSearchOpen(false); navigate(item.path); };
    ...
    <div className="search-overlay" onClick={() => setSearchOpen(false)}>
      <div className="search-modal" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
        <div className="search-input-wrap">
          <FiSearch size={20} />
          <input ref={inputRef} className="search-input" placeholder="검색어를 입력하세요... (ESC로 닫기)"
            value={query} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setQuery(e.target.value)} />
        </div>
        <div className="search-results">
          {query.length > 0 && results.length === 0 && <div className="search-empty">검색 결과가 없습니다</div>}
          {results.map((item, i) => (
